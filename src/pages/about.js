...
pages.about = {
	content: () => html
		`<div class = "pageContainer flexColumnTop extraExtraLargeGap mediumPadding">
			<div class = "halfWidth flexColumnTop largeGap">
			...
			${ constants.vocabularyErrors.scraperErrors.length > 0 || constants.vocabularyErrors.parserErrors.length > 0 ? html
				`<div class = "halfWidth flexColumnTop largeGap">
					...
					<div class = "flexColumnLeft smallGap">
						...
						${ constants.vocabularyErrors.parserErrors.map (error => html
							`<p class = "extraSmallFont">${ error }</p>`
						) }
					</div>
				</div>` :
